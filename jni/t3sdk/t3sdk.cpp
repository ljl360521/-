...
void RSACrypto::parsePEM(const std::string& pem) {
    ...
    int off=0;
    auto readLen=[&]()->int{
        if (off >= (int)der.size()) throw std::runtime_error("DER: truncated length");
        unsigned char f=der[(size_t)off++]; int n,l=0;
        if(f<0x80) return f;
        n=f&0x7F;
        if (n <= 0 || off + n > (int)der.size()) throw std::runtime_error("DER: invalid length");
        for(int i=0;i<n;i++){l=(l<<8)|der[(size_t)off];off++;}
        return l;
    };
    
    /* 外层SEQUENCE */
    if(off >= (int)der.size() || der[(size_t)off]!=0x30) throw std::runtime_error("DER: expected SEQUENCE");
    off++; readLen();
...
std::string CustomBase64::decode(const std::string& data) const {
    ...
    if (len <= 0) return out;
    while ((int)std64.size() % 4 != 0) std64 += '=';
    len = (int)std64.size();
...
std::once_flag g_t3_curl_once;

size_t t3WriteString(void* ptr, size_t size, size_t nmemb, void* userdata) {
...
std::string httpPostRaw(const std::string& url, const std::string& postData) {
    std::call_once(g_t3_curl_once, [](){ curl_global_init(CURL_GLOBAL_DEFAULT); });
    ...
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, t3WriteString);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, &response);
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, 15L);
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, 8L);
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "T3VerifyCpp/1.0");
    ...
    if (rc != CURLE_OK || httpCode < 200 || httpCode >= 300) return "";
    return response;
}
