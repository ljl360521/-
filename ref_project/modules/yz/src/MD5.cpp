...
// 便捷函数实现
void MD5String(const char* str, unsigned char digest[16])
{
    if (!str || !digest) return;
...
void MD5File(const char* filename, unsigned char digest[16], unsigned long* file_size)
{
    ...
    {
        MD5Update(&context, buffer, (unsigned int)bytes);
        total_bytes += bytes;
    }
...
void MD5HexToString(const unsigned char digest[16], char* output)
{
